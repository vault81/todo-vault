...
fn process_scroll_event() {
    ...
    tracing::info!("Scroll x: {} y: {}", scroll_x, scroll_y);

    let scroll_anchor_top_position =
        get_element_dom_rect("scroll-tbody").expect("Scroll tbody not found");
    tracing::info!(
        "Tbody top: {} bottom: {} height: {}",
        scroll_anchor_top_position.top(),
        scroll_anchor_top_position.bottom(),
        scroll_anchor_top_position.height()
    );

    let window_dimensions = get_window_dimensions();
    tracing::info!(
        ...
        <tr class="grid grid-cols-12 items-baseline my-4 bg-white rounded border md:table-row md:my-0 md:rounded-none md:border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 grid-rows-auto bak-grid-rows-3 min-w-[20rem] dark:hover:bg-gray-600">
            ...
            <td class="grid order-3 grid-cols-1 col-span-2 col-end-12 row-span-3 row-start-1 gap-6 justify-items-center justify-self-end self-center p-4 md:flex-row md:grid-cols-2 md:grid-rows-1 md:grid-rows-none md:px-2 asdasdgrid-rows-2">
                <FormDrawerButton
                    ...
                    icon=Svg::FileEdit
                    fields=vec![
                        FormField { id : "id".to_string(), label : None, input_type :
                        FormFieldInputType::Hidden, placeholder : "".to_string(), value : todo.id
                        .to_string(), required : true, }, FormField { id : "title".to_string(), label :
                        Some("Title".to_string()), input_type : FormFieldInputType::Text, placeholder :
                        "Do things".to_string(), value : todo.title, required : true, }, FormField { id :
                        "description".to_string(), label : Some("Description".to_string()), input_type :
                        FormFieldInputType::TextArea, placeholder : "".to_string(), value : todo.description
                        .unwrap_or("".to_string()), required : false, }, FormField { id : "due_date"
                        .to_string(), label : Some("Due Date".to_string()), input_type :
                        FormFieldInputType::Date, placeholder : "".to_string(), value : todo.due_date.map(|
                        dd | dd.format("%Y-%m-%d").to_string()).unwrap_or_else(|| "".to_string()), required :
                        false, },
                    ]
                />
        ...
        <div class="overflow-x-auto relative border-0 border-gray-200 shadow-md md:rounded-lg md:border dark:border-gray-700">
            <div class="flex justify-between items-center px-2 pt-2 pb-4">
                <FormDrawerButton
                    ...
                    icon=Svg::FilePlus
                    fields=vec![
                        FormField { id : "title".to_string(), label : Some("Title".to_string()), input_type :
                        FormFieldInputType::Text, placeholder : "Do things".to_string(), value : ""
                        .to_string(), required : true, }, FormField { id : "description".to_string(), label :
                        Some("Description".to_string()), input_type : FormFieldInputType::TextArea,
                        placeholder : "".to_string(), value : "".to_string(), required : false, }, FormField
                        { id : "due_date".to_string(), label : Some("Due Date".to_string()), input_type :
                        FormFieldInputType::Date, placeholder : "".to_string(), value : "".to_string(),
                        required : false, },
                    ]
                />
            ...
            <Transition fallback=move || {
                ...
                <Table column_headers=vec![
                    "Title".to_string(), "Description".to_string(), "Due Date".to_string(), "Action"
                    .to_string()
                ]>
                    <For
